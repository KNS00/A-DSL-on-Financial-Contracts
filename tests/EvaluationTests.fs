...
module evaloTests = 
    let E (s: string, t: int) = // test E function:, returns 0.0 if the stock is not found (just for testing)
        match s, t with
        | "AAPL", 1 -> 100.23
        | "AAPL", 3 -> 104.33
        | "AAPL", 5 -> 108.11
...
module evalccyTests =
    let stringToCurrency (curr : string) : Currency =
        match curr with
        | "EUR" -> EUR
        | "GBP" -> GBP
        | "DKK" -> DKK
    ...
    [<Theory>]
    [<InlineData(1.0,   "EUR",1.10)>]
    [<InlineData(25.5,  "EUR", 28.05)>]
    [<InlineData(1,     "GBP", 1.24)>]    
    [<InlineData(-5.6,  "GBP", -6.944)>]    
    [<InlineData(1,     "DKK", 0.15)>]
    [<InlineData(12.0,  "DKK", 1.8)>]
    [<InlineData(1.0,   "USD", 1.0)>]    
    [<InlineData(100.27,"USD", 100.27)>]    
    let ``evalccy should evaluate currencies correctly``(input, curr, expectedOutput) =
        let ccy = stringToCurrency curr
        let output = input * evalccy ccy
        let tolerance = 10e-7
